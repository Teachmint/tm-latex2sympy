...
def convert_atom(atom):
    if atom.atom_expr():
        ...
        var2 = get_differential_var(atom.DIFFERENTIAL())
        return sympy.Symbol('d' + var2.name, real=is_real)
    elif atom.mathit():
...
def handle_limit(func):
    sub = func.limit_sub()
    if sub.LETTER_NO_E():
        var2 = sympy.Symbol(sub.LETTER_NO_E().getText(), real=is_real)
    elif sub.GREEK_CMD():
        var2 = sympy.Symbol(sub.GREEK_CMD().getText()[1:].strip(), real=is_real)
    elif sub.OTHER_SYMBOL_CMD():
        var2 = sympy.Symbol(sub.OTHER_SYMBOL_CMD().getText().strip(), real=is_real)
    else:
    ...
    return sympy.Limit(content, var2, approaching, direction)
